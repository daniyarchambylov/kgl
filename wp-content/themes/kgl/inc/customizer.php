 ...
 */
function kgl_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'kgl_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'kgl' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => kgl_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'kgl' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'kgl' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'kgl' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'kgl' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'kgl' );
}
add_action( 'customize_register', 'kgl_customize_register', 11 );

/**
 * Register color schemes for Twenty Fifteen.
 *
 * Can be filtered with {@see 'kgl_color_schemes'}.
 *
 ...
 */
function kgl_get_color_schemes() {
	return apply_filters( 'kgl_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'kgl' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'kgl_get_color_scheme' ) ) :
/**
 ...
 */
function kgl_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = kgl_get_color_schemes();
...
endif; // kgl_get_color_scheme

if ( ! function_exists( 'kgl_get_color_scheme_choices' ) ) :
/**
 ...
 */
function kgl_get_color_scheme_choices() {
	$color_schemes                = kgl_get_color_schemes();
...
endif; // kgl_get_color_scheme_choices

if ( ! function_exists( 'kgl_sanitize_color_scheme' ) ) :
/**
 ...
 */
function kgl_sanitize_color_scheme( $value ) {
	$color_schemes = kgl_get_color_scheme_choices();
...
endif; // kgl_sanitize_color_scheme
...
function kgl_color_scheme_css() {
	...
	$color_textcolor_rgb         = kgl_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = kgl_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = kgl_get_color_scheme_css( $colors );

	wp_add_inline_style( 'kgl-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'kgl_color_scheme_css' );
 ...
 * @since Twenty Fifteen 1.0
 */
function kgl_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', kgl_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'kgl_customize_control_js' );
 ...
 * @since Twenty Fifteen 1.0
 */
function kgl_customize_preview_js() {
	wp_enqueue_script( 'kgl-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'kgl_customize_preview_js' );
 ...
 */
function kgl_get_color_scheme_css( $colors ) {
 ...
 * @since Twenty Fifteen 1.0
 */
function kgl_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-kgl-color-scheme">
		<?php echo kgl_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'kgl_color_scheme_css_template' );
